...
|Summary|사용자가 이 시스템을 처음 사용할 때 회원 가입을 위해 사용한다.|
|-------|-----------------------------|
|Scope|당신을 바꾸는 습관|
|Level|User Level|
|Author|신지예|
...
|3|사용자가 ID와 PW를 기입한다.|
|4|사용자가 회원가입 버튼을 누른다.|
|5|회원 정보가 데이터 베이스에 저장되고 로그인 창으로 이동한다.|
...
|1|사용자가 로그인 버튼을 누른다.|
|2|시스템이 로그인 창을 띄워준다.|
|3|사용자가 등록된 ID와 PW를 입력한다.|
|4|사용자가 로그인 버튼을 누른다.|
|5|시스템이 데이터베이스와 대조하여 일치함을 확인한다.|
|6|시스템이 해당 사용자의 데이터를 불러온다.|
|7|메인 화면이 출력된다.|

EXTENSION SCENARIO
|Step|Branching Action|
|--------|---------------------------|
|4|4a. ID 또는 PW를 입력하지 않은 경우|
||4a.1. ID 또는 PW가 입력되지 않았다는 메세지를 띄운다.|
||4a.2. 로그인 페이지로 돌아간다.|
|5|5a. 입력한 ID가 존재하지 않거나 PW가 일치하지 않는 경우|
||5a.1. ID 또는 PW가 틀렸는다 메세지를 띄운다.|
||5a.2. 로그인 페이지로 돌아간다.|
...
GENERAL CHARACTERISTICS
|Summary|현재 사용자의 변경사항을 저장한 후, 세션을 종료하고 가장 처음 화면을 띄운다|
|-------|-----------------------------|
|Scope|당신을 바꾸는 습관|
|Level|User Level|
|Author|신지예|
...
|--------|---------------------------|
|2|2a. 변경사항이 있는 경우|
||2a.1. 변경사항을 데이터베이스에 저장한다.|
||2a.a 데이터 저장 과정에서 오류가 발생한 경우|
||2a.a.1. 데이터 저장 중 오류가 발생했다는 메세지를 띄운다.|
||2a.a.2. 이전의 화면을 유지한다.|
...
|Step|Action|
|-------|----------------------------|
|1|사용자가 습관 추가 버튼을 누른다.|
|2|시스템이 습관 추가 화면을 띄워준다.|
...
|Step|Branching Action|
|--------|---------------------------|
|5|5a. 존재하는 습관과 이름이 동일한 경우|
||5a.1. 동일한 이름이 이미 존재한다는 메세지를 띄운다.|
||5b. 이름이 비어있는 경우|
||5b.1. 습관 이름을 입력해달라는 메세지를 띄운다.|
|6|6a. 데이터 저장 과정에서 오류가 발생한 경우|
||6a.1. 데이터가 저장되지 않았다는 메세지를 띄운다.|
||6a.2. 메인 화면으로 돌아간다.|
...
|--------|---------------------------|
|1|1a. 삭제할 습관이 없는 경우|
||1a.1. 삭제할 습관이 없다는 메세지를 띄우고 메인 화면으로 돌아간다.|
